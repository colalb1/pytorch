...
''')
    ...
    def test_linalg_matrix_sqrt(self, device, dtype):
        # General matrices with right-half-plane spectrum (real principal sqrt exists).
        def make_general(n, batch):
            a = make_tensor((*batch, n, n), dtype=dtype, device=device, low=-1, high=1) * 0.3
            if n > 0:
                a = a + 3 * torch.eye(n, dtype=dtype, device=device)
            return a

        for n, batch in itertools.product([0, 1, 5], [(), (3,), (2, 2)]):
            a = make_general(n, batch)
    ...
    def test_linalg_matrix_sqrt_autograd(self, device, dtype):
        # General input; gradcheck covers reverse and forward mode.
        a = make_tensor((4, 4), dtype=dtype, device=device, low=-1, high=1) * 0.3
        a = (a + 3 * torch.eye(4, dtype=dtype, device=device)).detach().requires_grad_(True)
        self.assertTrue(torch.autograd.gradcheck(torch.linalg.matrix_sqrt, (a,), check_forward_ad=True))

    @skipCPUIfNoLapack
    @dtypes(torch.double, torch.cdouble)
    def test_linalg_matrix_sqrt_grad_stable_at_degeneracy(self, device, dtype):
        # Non-symmetric input with eigenvalue 2 repeated: the Sylvester-based
        # backward stays finite at repeated eigenvalues.
        a = torch.tensor([
            [2.0, 0.7, 0.1, 0.0, 0.3],
            [0.0, 2.0, 0.4, 0.2, 0.0],
            [0.0, 0.0, 5.0, 0.6, 0.1],
            [0.0, 0.0, 0.0, 7.0, 0.5],
            [0.0, 0.0, 0.0, 0.0, 2.0],
        ], dtype=dtype, device=device).detach().requires_grad_(True)
        x = torch.linalg.matrix_sqrt(a)
        x.backward(torch.randn(5, 5, dtype=dtype, device=device))
    ...
    def test_linalg_matrix_sqrt_errors(self, device, dtype):
        ...
        # For complex inputs the principal (complex) square root is well-defined.
        if dtype.is_floating_point:
            indefinite = torch.diag(torch.tensor([1.0, -2.0], device=device)).to(dtype)
            with self.assertRaisesRegex(RuntimeError, "no real square root"):
                torch.linalg.matrix_sqrt(indefinite)
        else:
            indefinite = torch.diag(torch.tensor([1.0, -2.0], device=device)).to(dtype)
            x = torch.linalg.matrix_sqrt(indefinite)
            self.assertEqual(x @ x, indefinite, atol=2e-4, rtol=2e-4)

    @skipCPUIfNoLapack
    @dtypes(torch.float, torch.double, torch.cfloat, torch.cdouble)
    def test_linalg_matrix_sqrt_singular(self, device, dtype):
        # A singular matrix with a simple zero eigenvalue still has a square root.
