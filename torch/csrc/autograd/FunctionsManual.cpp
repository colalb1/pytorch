...
// Double backward for _cdist_backward. The first backward computes, for
// diff_ijk = x1_ik - x2_jk,
//   grad_x1_ik = sum_j grad_output_ij * sgn(diff)|diff|^(p-1) / cdist_ij^(p-1),
// so here `grad` is the upstream gradient wrt grad_x1 and we return the
// gradients wrt {grad_output, x1, x2, cdist}. `cdist` is treated as an
// independent input (its own dependence on x1/x2 is handled by the cdist slot
// flowing back through _cdist_forward's backward). All negative powers of
// |diff| or cdist are masked to zero at the exact zeros, matching the
// subgradient convention of the forward kernels.
std::tuple<Tensor, Tensor, Tensor, Tensor> _cdist_backward_backward(
  ...
  if (p == 0.0) {
    ...
    // Everything reduces to matmuls/reductions; no (r1, r2, m) materialization.
    ...
    // The first backward is piecewise constant in x1/x2/cdist, so only the
    // grad_output slot is (a.e.) nonzero.
    if (need_go) {
    ...
    // General p: materialize the (..., r1, r2, m) pairwise difference.
    auto diff = x1.unsqueeze(-2) - x2.unsqueeze(-3);
    auto adiff = diff.abs();
    ...
    if (need_go || need_cdist) {
      ...
      P = ((grad.unsqueeze(-2) * signpow).sum(-1) / cdist_pow).masked_fill(zero, 0);
    }
    if (need_go)
      grad_grad_output = P;
    if (need_cdist)
    ...
    if (need_x1 || need_x2) {
      auto weighted = W.unsqueeze(-1) * adiff.pow(p - 2).masked_fill(diff_zero, 0);
      if (need_x1)
...
// Double backward for _pdist_backward. pdist over `self` (n, m) is cdist of
// `self` with itself restricted to the unordered pairs, packed row-major over
// the strict upper triangle. We rebuild the symmetric (n, n) grad_output/pdist
// matrices, reuse the cdist double backward with x1 == x2 == self, and fold the
// results back to the packed layout. Because `self` appears as both endpoints,
// its gradient is the sum of the x1 and x2 slots.
